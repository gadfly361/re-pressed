...
(defn register-subs [event-type]
  (let [ns-keyword (->ns-keyword event-type)]
    (rf/reg-sub
      (ns-keyword "-keys")
      (fn [db _]
        (get-in db [:keyboard (ns-keyword) :keys])))

    (rf/reg-sub
      (ns-keyword "-event-keys")
      (fn [db _]
        (get-in db [:keyboard (ns-keyword) :event-keys])))

    (rf/reg-sub
      (ns-keyword "-clear-keys")
      (fn [db _]
        (get-in db [:keyboard (ns-keyword) :clear-keys])))

    (rf/reg-sub
      (ns-keyword "-always-listen-keys")
      (fn [db _]
        (get-in db [:keyboard (ns-keyword) :always-listen-keys])))

    (when (= "keydown" event-type)
      (rf/reg-sub
        (ns-keyword "-prevent-default-keys")
        (fn [db _]
          (get-in db [:keyboard (ns-keyword) :prevent-default-keys]))))))
...
(defn- ->set-key! [event-type]
  (let [ns-keyword (->ns-keyword event-type)]
    (fn set-key!
      [{:keys [db]} [_ key-map]]
      {:db (update-in db [:keyboard (ns-keyword) :keys]
                      (fn [key-maps]
                        (let [keys (if key-map
                                     (conj (or key-maps
                                               [])
...
(defn- ->clear-keys! [event-type]
  (let [ns-keyword (->ns-keyword event-type)]
    (fn clear-keys!
      [{:keys [db]} [_ key-map]]
      {:db (assoc-in db [:keyboard (ns-keyword)
                         :keys] [])})))
